...
然后再回到jspang.html文件，用全部地替换的方法，把%换车$，这样就更换成功了。建议这个还是在项目初级就规范好，不要随意改动。  
...
比如现在我们到/app/controller/jspang.js文件里，修改add( )方法，把maxAge设置为两秒，在添加Cookie两秒后，这个Cookie就会自己失效。  
